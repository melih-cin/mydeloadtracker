-- MyDeloadTracker: apply migrations 0003–0009 in one paste.
-- Prod already has 0001,0002,0004,0005,0006; 0003 (daily_checkins) was skipped,
-- so 0007 needs it. Fully idempotent + re-run-safe; paste into Supabase SQL Editor.
begin;

-- prelude: guarantee the equipment column + global-name unique index exist BEFORE
-- any seed inserts, so re-running the (already-applied) exercise seeds can never dup.
alter table public.exercises add column if not exists equipment text;
create unique index if not exists exercises_global_name_uidx on public.exercises (name) where user_id is null;
